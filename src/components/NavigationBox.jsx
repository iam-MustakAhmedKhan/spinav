...
const NavigationBox = () => {
    ...
    return (
        <motion.div
            ...
            variants={variants}
            layout
            className={`absolute divclass bg-[#EDF6FD] h-[50%] w-full ${
                pathName ? "overflow-y-auto" : ""
            } bottom-0 rounded-t-[26px] p-5 pt-0   scrollbarHide`}
            {...swipeHandler}
        >
            {isOpen || isScroll || pathName !== "" ? (
                ""
            ) : (
                <div className="w-[4rem] h-[6px] rounded-md mx-auto bg-[#051B29]/[0.2] mb-0 mt-2"></div>
            )}
            <div
                className="searchBox fixed bg-[#EDF6FD] z-30 py-2"
                style={{ width: `calc(100% - 40px)` }}
            >
                <div className="mx-auto">
                    <div
                        className={`relative ${
                            isOpen || isScroll || pathName !== "" ? "mt-2" : ""
                        } rounded-full flex items-center w-full h-12 bg-[#E6EFF6] overflow-hidden`}
                    >
                        {isOpen || isScroll || pathName !== "" ? (
                            <button
                                className="text-2xl font-bold ml-2 text-[#051B29]/[0.5]"
            ...
            {searchText == "" && location == "/" && (
                <div
                    className={`navigationBoxes grid grid-cols-2 gap-5 items-center  justify-between ${
                        isOpen || isScroll ? "mt-[5rem] " : "mt-[4.5rem]"
                    } pb-24 `}
                >
                    {datas.map((data) => (
            ...
            {searchValue !== "" && (
                <div className="mt-[4.5rem] overflow-y-auto scrollbarHide h-[calc(100vh-4.5rem)] pb-[10rem]">
                    <ul
                        className={`${
                            isClick ? "hidden" : "flex"
                                    ...
                                    return room.roomNo
                                        ...
                                        .map((v, i) => (
                                            <Link
                                                key={i}
                                                onClick={() =>
                                                    onClickHandler(v.roomname)
                                                }
                                                to={`/${
                                                    data.title
                                                }/${v.roomname.replaceAll(
                                                    " ",
                                                    "-"
                                                )}`}
                                            >
                                                <li className=" p-3 first:rounded-t-[6px] last:rounded-b-[6px] bg-[#E6EFF6] mb">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <p className="ml-0 text-left font-semibold">
                                                            {v.roomname}
                                                        </p>
                                                    </div>
                                                </li>
                    ...
                    {datas.map((data) =>
                        data.individual.filter(handleSearch).map((v, i) => (
                            <Link
                                key={i}
                                to={`/${data.title}/${v.roomname.replaceAll(
                                    " ",
                                    "-"
                                )}`}
                                onClick={() => onClickHandler(v.roomname)}
                            >
                                <div
                                    className={`bg-[#E6EFF6] ${
                                        isClick ? "hidden" : "flex"
                                    } items-center px-2 py-2 rounded-[12px] gap-y-2 mb-2 mt-2`}
                                >
